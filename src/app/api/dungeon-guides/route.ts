...
import { buildCreateData, canManageGuildContent, createDungeonGuideSchema, serializeBuild } from "@/lib/content-api";
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ canManage: canManageGuildContent(member.role), guides: guides.map((guide) => ({ ...guide, author: guide.author.nickname, createdAt: guide.createdAt.toISOString(), updatedAt: guide.updatedAt.toISOString(), monsters: guide.monsters.map(serializeBuild) })) });
}
