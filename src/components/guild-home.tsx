    ...
    <AppShell activeSection="home">
      ...
      <div className="home-grid">
        ...
        <section className="home-panel home-homework">
          ...
          {data?.homeworks.map((item) => <article className="homework-preview" key={item.id}><div className="homework-preview-copy"><span className="status-pill">{item.completedByMe ? "내 숙제 완료" : "진행 중"}</span><h3>{item.title}</h3><p>{item.target}</p><small>{item.dueAt ? `${formatDate(item.dueAt)} 마감` : "기한 없음"} · 작성 {item.author}</small><KakaoShareButton category="길드 숙제" description={`${item.target} · ${item.dueAt ? `${formatDate(item.dueAt)} 마감` : "기한 없음"}`} path="/homeworks" title={item.title} /></div><BuildSummary builds={item.monsters} /></article>)}
        </section>
