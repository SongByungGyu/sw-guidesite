...
export function DungeonGuides() {
  ...
  const load = useCallback(() => fetch(`/api/dungeon-guides?dungeon=${encodeURIComponent(selectedKey)}`, { cache: "no-store" }).then((response) => response.json()).then((result: { guides: Guide[] }) => setGuides(result.guides)), [selectedKey]);
  useEffect(() => { void load(); }, [load]);

  function chooseCategory(next: DungeonCategory) { setCategory(next); const first = dungeonCatalog.find((item) => item.category === next); if (first) setSelectedKey(first.key); setCreating(false); setBuilds([]); }
  function chooseDungeon(key: string) { const next = dungeonByKey.get(key); if (next) setCategory(next.category); setSelectedKey(key); setCreating(false); setBuilds([]); }
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/dungeon-guides", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dungeonKey: selected.key, title: form.get("title"), summary: form.get("summary"), strategy: form.get("strategy"), difficulty: form.get("difficulty"), clearTime: form.get("clearTime"), builds }) });
  ...
  return <AppShell activeSection="dungeons">
    <div className="page-heading"><div><p className="eyebrow">PVE ARCHIVE</p><h1>던전 공략</h1><p>카이로스·2차 각성·차원의 홀·이계 레이드까지 길드의 실제 스펙을 모읍니다.</p></div><button className="button primary" onClick={() => setCreating((value) => !value)} type="button"><Icon name={creating ? "x" : "plus"} size={18} /> {creating ? "작성 닫기" : "공략 등록"}</button></div>
    ...
    <div className="dungeon-layout"><aside className={`dungeon-list${query ? " is-searching" : ""}`}><header><strong>{query ? "검색 결과" : dungeonCategories.find((item) => item.key === category)?.name}</strong><span>{items.length}개 콘텐츠</span></header>{items.map((item) => <button aria-pressed={selectedKey === item.key} key={item.key} onClick={() => chooseDungeon(item.key)} type="button"><span className="dungeon-icon">{item.shortName.slice(0, 1)}</span><span><strong>{item.name}</strong><small>{item.reward}</small></span>{item.badge ? <em>{item.badge}</em> : null}<Icon name="chevron" size={17} /></button>)}</aside>
      <section className="dungeon-content"><header className="dungeon-hero"><div><span>{dungeonCategories.find((item) => item.key === selected.category)?.name}</span><h2>{selected.name}</h2><p>{selected.description}</p></div><dl><div><dt>주요 보상</dt><dd>{selected.reward}</dd></div><div><dt>권장 편성</dt><dd>{selected.teamSize}마리</dd></div></dl></header>
        {creating ? <form className="content-create-form compact" onSubmit={submit}><header><div><span className="step-badge">+</span><div><h2>{selected.name} 공략 등록</h2><p>실제 클리어 덱과 최소 스펙을 공유하세요.</p></div></div></header><div className="simple-fields guide-fields"><label><span>공략 제목</span><input name="title" required minLength={2} placeholder="예: 평균 50초 안정덱" /></label><label><span>한 줄 요약</span><input name="summary" required minLength={5} placeholder="누가 사용하면 좋은 덱인지 요약" /></label><label><span>난이도·층</span><input name="difficulty" placeholder="예: 심연 Hard" /></label><label><span>평균 기록</span><input name="clearTime" placeholder="예: 48~55초" /></label><label className="span-full"><span>상세 운용법</span><textarea name="strategy" required minLength={10} placeholder="턴 순서, 보스 기믹 대응, 대체 몬스터를 적어주세요." /></label></div><TeamBuildEditor builds={builds} onChange={setBuilds} teamSize={selected.teamSize} />{error ? <p className="form-error">{error}</p> : null}<footer><button className="button secondary" type="button" onClick={() => setCreating(false)}>취소</button><button className="button primary" type="submit">공략 저장</button></footer></form> : null}
        <div className="guide-list-heading"><div><h2>길드 공략</h2><p>{guides.length ? `${guides.length}개의 검증된 편성` : "아직 등록된 공략이 없습니다."}</p></div></div>{guides.length ? <div className="guide-list">{guides.map((guide) => <article className="guide-card" key={guide.id}><header><div><span className="status-pill">{guide.difficulty || "일반"}</span><h3>{guide.title}</h3><p>{guide.summary}</p></div><div><strong>{guide.clearTime || "기록 없음"}</strong><small>작성 {guide.author}</small></div></header><BuildDetails builds={guide.monsters} /><details><summary>상세 운용법 보기</summary><p>{guide.strategy}</p></details></article>)}</div> : <div className="request-empty dungeon-empty"><Icon name="book" /><strong>{selected.name} 첫 공략을 등록해 보세요.</strong><p>몬스터와 스펙을 함께 저장하면 길드원이 바로 따라 할 수 있습니다.</p></div>}</section></div>
  </AppShell>;
}
