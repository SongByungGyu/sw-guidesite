...
export function HomeworkBoard() {
  ...
  return <AppShell activeSection="homeworks"><div className="page-heading"><div><p className="eyebrow">GUILD MISSION</p><h1>길드 숙제</h1><p>운영진이 지정한 공덱과 추천 공격 대상을 한 화면에서 확인합니다.</p></div>{canCreate ? <button className="button primary" onClick={() => creating ? closeEditor() : setCreating(true)} type="button"><Icon name={creating ? "x" : "plus"} size={18} /> {creating ? "작성 닫기" : "숙제 작성"}</button> : <span className="permission-badge"><Icon name="shield" size={16} /> 운영진만 작성 가능</span>}</div>
    ...
    <div className="homework-board-list">{homeworks.map((homework) => <article className={`homework-card${homework.completedByMe ? " is-completed" : ""}`} key={homework.id}><header><div><span className="status-pill">{homework.completedByMe ? "내 숙제 완료" : homework.status === "ACTIVE" ? "진행 중" : "종료"}</span><h2>{homework.title}</h2><p>작성 {homework.author}</p></div><time>{homework.dueAt ? `${new Intl.DateTimeFormat("ko-KR", { month: "short", day: "numeric", weekday: "short" }).format(new Date(homework.dueAt))} 마감` : "기한 없음"}</time></header><section className="homework-target"><span><Icon name="search" size={17} /> 추천 공격 대상</span><strong>{homework.target}</strong></section><HomeworkBuildTable builds={homework.monsters} />{homework.progress ? <HomeworkProgressPanel progress={homework.progress} /> : null}<footer><div><strong>운용 지시</strong><p>{homework.strategy}</p></div><div className="homework-footer-actions">{canCreate ? <><button className="button secondary" disabled={deletingId === homework.id} onClick={() => startEdit(homework)} type="button"><Icon name="edit" size={17} /> 숙제 수정</button><button className="button secondary danger" disabled={deletingId === homework.id} onClick={() => void deleteHomework(homework)} type="button"><Icon name="trash" size={17} /> {deletingId === homework.id ? "삭제 중" : "숙제 삭제"}</button></> : null}<button aria-pressed={homework.completedByMe} className={`button ${homework.completedByMe ? "success" : "secondary"}`} disabled={savingCompletion === homework.id || deletingId === homework.id} onClick={() => void toggleComplete(homework)} type="button"><Icon name="check" size={17} /> {savingCompletion === homework.id ? "저장 중" : homework.completedByMe ? "완료 취소" : "완료 표시"}</button></div></footer></article>)}</div>
  </AppShell>;
}
